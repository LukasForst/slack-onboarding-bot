...
def register(url: str, email: str, password: str, service_name: str) -> bool:
    ...
    r = requests.post(f"{url}/register", json=payload)
    if r:
...
def login(url: str, email: str, password: str) -> bool:
    ...
    r = requests.post(f"{url}/login", json=payload)
    if r:
...
def create_service(url: str, name: str, service_url: str) -> Optional[str]:
    payload = {
        'name': name,
        'url': service_url
    }
    r = requests.post(f"{url}/service", json=payload)
    if r:
...
def update_service_url(url: str, service_url: str):
    r = requests.put(f'{url}/service', json={'url': service_url})
    if r:
    ...
    if r:
        ...
        if auth_code:
            ...
            if service_url and json['webhook'] != service_url:
                print(f'Current service URL: {json["webook"]} does not match provided: {service_url}. Updating.')
...
def obtain_auth(url: str, email: str, password: str,
                service_name: Optional[str] = None, service_url: Optional[str] = None) -> Optional[str]:
    ...
    if not auth_code:
        auth_code = create_service(url, service_name, service_url)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--service-url", "-s", help="URL where is the service running.")

    args = parser.parse_args()
    auth = obtain_auth(args.url, args.email, args.password, args.name, args.service_url)
    if auth:
